...
app.get("/display",(req,res) => {
  database.query(`SELECT * FROM mca_stud`,function(err, result) {    
...
app.get("/addstudent",(req,res) => {
  ...
  //Sanitization XSS...
  let qry = "select * from mca_stud where regno=?";
...
res.render("add",{ checkmesg:true })
          ...
          //insert query
          let qry2 = "insert into mca_stud values(?,?,?,?,?)";
...
app.get("/removestudent", (req, res) => {
  ...
  let qry = "delete from mca_stud where regno=?";
  database.query(qry, [regno], (err, results) => {
      if (err) throw err
      else {
          if (results.affectedRows > 0) {
...
app.get("/updatestudent", (req,res) => {
  ...
  let qry = "update  mca_stud set name=?, Institutional_Email=?, Personal_Email=?, Mobile=? where regno=?";
